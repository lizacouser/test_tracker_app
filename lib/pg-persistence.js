...
module.exports = class PgPersistence {
  ...
  async addBaseline(studentId, testType) {
    const DELETE_EXISTING_BASELINE = `DELETE FROM students_tests WHERE student_id = $1 AND username = $2;`;
    await dbQuery(DELETE_EXISTING_BASELINE, studentId, this.username);
    ...
    const ADD_NEW_BASELINE_TEST = `INSERT INTO students_tests (test_id, student_id, done, username)
                        VALUES ($1, $2, true, $3);`;
    ...
    let addedTest = await dbQuery(ADD_NEW_BASELINE_TEST, testId, studentId, this.username);
    ...
    const LOAD_BASELINE = `SELECT id AS students_tests_id FROM students_tests WHERE student_id = $1 AND username = $2`;
    let baseline = await dbQuery(LOAD_BASELINE, studentId, this.username);
    // console.log('grabbed the students_tests_id of the baseline with a given student_id');


    return baseline.rows[0].students_tests_id;
  }
  ...
  async addTestPack(studentId, packId) {
    ...
    let queryText = `INSERT INTO students_tests (test_id, student_id, username) VALUES `;

    let values = [];
    let allStandIns = [];
    let count = 1;
    testPack.forEach(test => {
      let standIns = [];
      values.push(test.id, studentId, this.username);
      [1,2,3].forEach(() => {
        standIns.push(`$${count}`);
        count++;
      });
      allStandIns.push(`(${standIns.join(', ')})`);
    });

    const ADD_TEST = queryText + allStandIns.join(', ') + ';';

    let addedTests = await dbQuery(ADD_TEST, ...values);

    if (addedTests.rowCount === 0) return undefined;
    return true;
  }
  ...
  //   const LOAD_TESTS =  `SELECT id FROM tests
  //                         WHERE pack_id = $1;`;
  //   const ADD_TEST = `INSERT INTO students_tests (test_id, student_id, username)
  //                       VALUES ($1, $2, $3);`;
  ...
  //     let addedTest = await dbQuery(ADD_TEST, testId, studentId, this.username);
  //     if (addedTest.rowCount === 0) return undefined;
  //   }
  ...
  async allTests(student) {
    ...
    const SORTED_TESTS = `SELECT t.title AS title,
                            ...
                            WHERE st.student_id = $1 AND p.title NOT ILIKE '%baseline%' AND
                            s.username = $2
                            ORDER BY st.done, st.date_completed DESC, p.test_type ${displayOrder}, p.order, t.id`;
    let resultTests = await dbQuery(SORTED_TESTS, student.id, this.username);
  ...
  async createStudent(name, testPlan) {
    const CREATE_STUDENT = `INSERT INTO students (name, test_plan, username) VALUES ($1, $2, $3)`;
    let createdStudent = await dbQuery(CREATE_STUDENT, name, testPlan, this.username);
    // console.log('inserted student into table');


    if (createdStudent.rowCount !== 1) return undefined;

    const LOAD_STUDENT = `SELECT * FROM students WHERE name LIKE $1 AND username = $2`;
    let newStudent = await dbQuery(LOAD_STUDENT, name, this.username);
  ...
  //                           AND st.done = true
  //                           AND p.test_type = $2 AND
  //                           s.username = $3
  //                           ORDER BY p.test_type ASC, p.order DESC;`;

  //   let resultPack = await dbQuery(LOAD_CURRENT_PACK, studentId, testPlan, this.username);
  ...
  async deleteStudent(studentId) {
    const DELETE_STUDENT = `DELETE FROM students WHERE id = $1 AND username = $2`;

    let deletedStudent = await dbQuery(DELETE_STUDENT, studentId, this.username);
  ...
  async editStudentInfo(studentId, name, testPlan) {
    const UPDATE_STUDENT = `UPDATE students SET name = $1, test_plan = $2
                            WHERE id = $3 AND
                            s.username = $4`;
    let updateResult = await dbQuery(UPDATE_STUDENT, name, testPlan, studentId, this.username);
  ...
  async highestTestScore(studentId) {
    ...
    let score = await dbQuery(LOAD_HIGH_SCORE, studentId, this.username);
  ...
  async loadScore(scoreId) { // new
    ...
    let score = await dbQuery(LOAD_SCORE, scoreId, this.username);
    if (score.rowCount === 0) return null;
    // console.log('load score result', score.rows[0]);
    return score.rows[0];
  }
  ...
  async loadStudent(studentId) {
    const LOAD_STUDENT = `SELECT * FROM students WHERE id = $1 AND username = $2`;
    const LOAD_BASELINE = `SELECT t.id AS test_id,
                            ...
                            JOIN packs p ON t.pack_id = p.id
                            LEFT JOIN scores sc ON sc.students_tests_id = st.id
                            WHERE st.student_id = $1 AND p.title ILIKE '%baseline%' AND
                            s.username = $2;`;
    ...
    let resultStudent = dbQuery(LOAD_STUDENT, studentId, this.username);
    let resultBaseline = dbQuery(LOAD_BASELINE, studentId, this.username);
    let resultTests = dbQuery(LOAD_TESTS, studentId, this.username);
  ...
  async loadTest(studentId, testId) {
    ...
    let resultTest = await dbQuery(LOAD_TEST, studentId, testId, this.username);
  ...
  async markAllDone(studentId) {
    ...
    let toggleResult = await dbQuery(MARK_ALL_DONE, studentId, this.username);
  ...
  async markAllUndone(studentId) {
    ...
    const CLEAR_ALL_SCORES = `DELETE FROM scores
                              WHERE students_tests_id IN (
                                ...
                                username = $2 AND test_id NOT IN (
                                  SELECT id FROM tests
                                  WHERE title ILIKE '%baseline%')
                                );`;

    let toggleResult = dbQuery(MARK_ALL_UNDONE, studentId, this.username);
    let clearResult = dbQuery(CLEAR_ALL_SCORES, studentId, this.username);
  ...
  async nextPacks(studentId, testPlan) {
    const LOAD_STUDENT_PACKS = `SELECT DISTINCT p.id AS id, p.title AS title
                                ...
                                AND p.id NOT IN (
                                  SELECT DISTINCT t.pack_id FROM tests t
                                  JOIN students_tests st ON st.test_id = t.id
                                  WHERE st.student_id = $2 AND
                                  st.username = $3
                                )
                                AND p.title NOT ILIKE '%baseline%'
                                ORDER BY p.id, p.title`;

    let resultPacks = await dbQuery(LOAD_STUDENT_PACKS, testPlan,studentId, this.username);
  ...
  async removeTestPack(studentId, packId) {
    ...
    let removedTests = await dbQuery(REMOVE_TESTS, studentId, this.username, packId);
  ...
  async sortedStudents() {
    const ALL_STUDENTS = `SELECT * FROM students WHERE username = $1 ORDER BY lower(name) ASC`;
    const ALL_TESTS = `SELECT t.title AS title,
                        ...
                        WHERE s.username = $1
                        ORDER BY p.order`;
    const ALL_BASELINE_SCORES = `SELECT scores.mock,
                        scores.projected,
                        scores.act_english,
                        scores.act_math,
                        scores.act_reading,
                        ...
                        t.title AS test,
                        p.test_type,
                        scores.students_tests_id
                 FROM scores
                 JOIN students_tests st ON scores.students_tests_id = st.id
                 JOIN tests t ON st.test_id = t.id
                 JOIN packs p ON t.pack_id = p.id
                 WHERE st.username = $1
                 AND t.title ILIKE '%baseline%';`;
    let resultStudents = dbQuery(ALL_STUDENTS, this.username);
    let resultTests = dbQuery(ALL_TESTS, this.username);
    let resultBaselines = dbQuery(ALL_BASELINE_SCORES, this.username);
    let resultAll = await Promise.all([resultStudents, resultTests, resultBaselines]);

    let allStudents = resultAll[0].rows;
    let allTests = resultAll[1].rows;
    let allBaselines = resultAll[2].rows;
    console.log(allBaselines);
    if (!allStudents || !allTests || !allBaselines) return undefined;

    allStudents.forEach(student => {
      let studentTestsWithBaseline = allTests.filter(test => {
        return test.student_id === student.id;
      });

      let baseline = studentTestsWithBaseline.find(test => test.title.includes("Baseline"));

      student.baselineScore = allBaselines.find(score => {
        return score.students_tests_id === baseline.students_tests_id;
      });

      student.tests = studentTestsWithBaseline.filter(test => {
        return !test.title.includes("baseline");
      });
    });
  ...
  //                       JOIN packs p ON t.pack_id = p.id
  //                       LEFT JOIN scores sc ON sc.students_tests_id = st.id
  //                       WHERE st.student_id = $1 AND
  //                       s.username = $2
  //                       ORDER BY p.order`;
  //   let resultStudents = await dbQuery(ALL_STUDENTS, this.username);
  //   let allStudents = resultStudents.rows;

  //   for (let index = 0; index < allStudents.length; index += 1) {
  //     let student = allStudents[index];
  //     let resultTests = await dbQuery(ALL_TESTS, student.id, this.username);
  ...
  async sortedTests(student) {
    const SORTED_TESTS = `SELECT t.title AS title,
                            ...
                            JOIN packs p ON p.id = t.pack_id
                            LEFT JOIN scores sc ON sc.students_tests_id = st.id
                            WHERE st.student_id = $1 AND p.test_type = s.test_plan AND
                            s.username = $2
                            AND p.title NOT ILIKE '%baseline%'
  ...
  async studentPacks(studentId, testPlan) {
    const LOAD_STUDENT_PACKS = `SELECT DISTINCT p.id AS id, p.title AS title
                                ...
                                JOIN tests t ON t.pack_id = p.id
                                JOIN students_tests st ON st.test_id = t.id
                                WHERE st.student_id = $1 AND p.test_type = $2 AND
                                st.username = $3
                                AND p.title NOT ILIKE '%baseline%'
                                ORDER BY p.id DESC, p.title`;

    let resultPacks = await dbQuery(LOAD_STUDENT_PACKS, studentId, testPlan, this.username);
    if (resultPacks.rowCount === 0) return undefined;
  ...
  async toggleDoneTest(studentId, testId) {
    ...
    let toggleResult = await dbQuery(TOGGLE_DONE_TEST, studentId, testId, this.username);
    return toggleResult.rowCount > 0;
  }
